...
const DetailItemModal = ({ showDetailItemModal, detailItem, onModalClose, onAddToList, onDownloadClick }) => {
    ...
    const detailItemModalTopRef = useRef(null);

    useEffect(() => {
        if (showDetailItemModal && detailItemModalTopRef.current) {
            detailItemModalTopRef.current.scrollIntoView();
        }
    }, [showDetailItemModal]);

    return (
        <div className={`${showDetailItemModal ? 'block' : 'hidden'} fixed top-0 left-0 w-screen min-h-screen z-20 bg-black/70`}
            ...
            {/* ====== Modal Content ====== */}
            <div className='pt-0 pb-16 h-screen overflow-auto'>

                <div ref={detailItemModalTopRef} className='h-20' />
